...
def parse_time_utc_to_est_hour(full_date_utc: date, time_val) -> tuple[date, int]:
    if isinstance(time_val, datetime):
        dt_utc = time_val.astimezone(UTC).replace(tzinfo=UTC)
        ...
        hh = (seconds // 3600) % 24; mm = (seconds % 3600) // 60; ss = seconds % 60
        dt_utc = datetime(full_date_utc.year, full_date_utc.month, full_date_utc.day, hh, mm, ss, tzinfo=UTC)
...
def fetch_facilities_all():
    ...
    for row in data:
        fid = row.get("id"); nm = row.get("name") or ""
        all_names.append((str(nm), fid))
        by_norm[norm_text(nm)].append(fid)
    exact_unique: dict[str, str] = {k: v[0] for k, v in by_norm.items() if len(v) == 1}
    print(f"📌 Facilities loaded: {len(data)} | unique exact-name keys: {len(exact_unique)}")
    return all_names, exact_unique
...
def load_rows_to_capacity_tables(rows: list[dict]):
    ...
    df["Received Time"] = pd.to_datetime(df["Received Time"], utc=True)
...
def _load_df_to_capacity_tables_core(df: pd.DataFrame):
    ...
    all_fac_names, exact_by_norm = fetch_facilities_all()
    cache = load_cache()
    ...
    for idx, row in df.iterrows():
        try:
            ...
            est_date, est_hour = parse_time_utc_to_est_hour(full_date_utc, row.get("Received Time"))
            ...
            if pd.isna(excel_fac_name) or not str(excel_fac_name).strip():
                fuzzy_misses.append({"row": idx, "excel_name": str(excel_fac_name), "score": None, "reason": "Missing Facility Name"})
                continue

            facility_uuid, matched_name, score, decision = resolve_facility_uuid_name_only(
                str(excel_fac_name), all_fac_names, exact_by_norm, cache
            )
            ...
            if facility_uuid:
                ...
                if decision in {"fuzzy_auto", "fuzzy_soft"}:
                    fuzzy_hits.append({
                        "row": idx, "excel_name": str(excel_fac_name),
                        "matched_db_name": matched_name, "score": score,
                        "decision": decision, "facility_id": facility_uuid
                    })
            else:
                fuzzy_misses.append({
                    "row": idx, "excel_name": str(excel_fac_name),
    ...
    for est_date_, hours in hours_by_date.items():
        ...
        supabase.table("capacity_per_hour").delete().eq("date", str(est_date_)).in_("hour", hours_list).execute()

    for (est_day, est_hour), total_rvus in sorted(rvus_by_hour.items()):
        supabase.table("capacity_per_hour").insert({
            "date": str(est_day), "hour": int(est_hour), "total_rvus": round(float(total_rvus), 2),
        }).execute()
        print(f"✅ (global) {est_day} - {est_hour:02d}:00 → {total_rvus:.2f} RVUs")

    # Facility deletes/inserts
    for est_date_, fac_hours in fac_hours_to_delete.items():
        for facility_id, hours in fac_hours.items():
            hours_list = sorted(hours)
            print(f"🧹 (facility) Deleting date={est_date_}, facility={facility_id}, hours={hours_list} (all modalities)")
            supabase.table("facility_capacity_per_hour").delete() \
                .eq("date", str(est_date_)).eq("facility_id", facility_id).in_("hour", hours_list).execute()

    for (est_date_, est_hour, facility_id, modality), total_rvus in sorted(fac_mod_rvus.items()):
        supabase.table("facility_capacity_per_hour").insert({
            "date": str(est_date_), "hour": int(est_hour), "facility_id": facility_id,
            "modality": modality, "total_rvus": round(float(total_rvus), 2),
        }).execute()
        print(f"✅ (facility) {est_date_} {est_hour:02d}:00 | {facility_id} [{modality}] → {total_rvus:.2f} RVUs")
